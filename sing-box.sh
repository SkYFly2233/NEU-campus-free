...
SB_USER_ADMIN_PAGE_B64
  # 独立脚本内置网页模板后，补齐用户表的上传、下载与个人总流量三列。
  # replace 仅匹配旧版片段，重复执行安装/升级也是安全的。
  python3 -c 'from pathlib import Path; import sys; p=Path(sys.argv[1]); s=p.read_text(encoding="utf-8"); s=s.replace("min-width: 940px", "min-width: 1020px").replace("<th>总流量</th>", "<th title=\"上传流量与下载流量之和\">个人总流量</th>").replace("<th>下载</th><th>已用 / 限额</th>", "<th>下载</th><th title=\"上传流量与下载流量之和\">个人总流量</th><th>已用 / 限额</th>").replace("colspan=\"7\" class=\"loading\"", "colspan=\"8\" class=\"loading\"").replace("colspan=\"7\" class=\"empty\"", "colspan=\"8\" class=\"empty\"").replace("<td class=\"number\">${formatBytes(user.upload_bytes)}</td><td class=\"number\">${formatBytes(user.download_bytes)}</td>\\n            <td><div class=\"quota\">", "<td class=\"number\">${formatBytes(user.upload_bytes)}</td><td class=\"number\">${formatBytes(user.download_bytes)}</td><td class=\"number\">${formatBytes(user.used_bytes)}</td>\\n            <td><div class=\"quota\">"); p.write_text(s, encoding="utf-8")' "${WORK_DIR}/admin-page.html"
